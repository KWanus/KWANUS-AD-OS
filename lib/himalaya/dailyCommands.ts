  ...
  try {
    ...
    if (!hasAssets) {
      commands.push({
        id: `cmd-${cmdId++}`,
        priority: 1,
        action: "Start your business in Himalaya",
        details: "Answer one question and Himalaya builds everything for you — site, ads, emails, the works.",
        estimatedTime: "2 min",
        category: "create",
        href: "/himalaya",
        completed: false,
    ...
    if (publishedSites.length === 0 && sites.length > 0) {
      commands.push({
        id: `cmd-${cmdId++}`,
        priority: 1,
        action: "Publish your site",
        details: `"${sites[0].name}" is built but not live. Publish it now so people can find you.`,
        estimatedTime: "1 min",
        category: "review",
    ...
    if (publishedSites.length > 0 && activeCampaigns.length === 0) {
      ...
      commands.push({
        ...
        details: "Share your new site. This is your first traffic.",
        estimatedTime: "3 min",
        category: "post",
      ...
      commands.push({
        id: `cmd-${cmdId++}`,
        priority: 2,
        action: "Set up your first ad campaign",
        details: "Your site is live. Now let's get strangers to it. Open campaigns and push your first ad.",
    ...
    if (emailFlows.length > 0 && emailFlows.every(f => f.status !== "active")) {
      commands.push({
        id: `cmd-${cmdId++}`,
        priority: 2,
        action: "Turn on your email automation",
        details: "You have email flows built but none are running. Activate at least one so leads get followed up automatically.",
        estimatedTime: "2 min",
        category: "email",
        href: "/emails",
        completed: false,
    ...
    if (publishedSites.length > 0) {
      ...
      commands.push({
        ...
        details: "Consistency builds audience. Post this or something similar.",
        estimatedTime: "2 min",
        category: "post",
