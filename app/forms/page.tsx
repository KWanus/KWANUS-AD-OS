...
function CreateFormModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-3xl border border-white/[0.08] bg-t-bg shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Free Strategy Call"
              className="w-full bg-white/[0.03] border border-white/[0.07] rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-[#f5a623]/30" />
          </div>
          <div>
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-white/[0.06]">
          ...
          <button onClick={() => void handleCreate()} disabled={creating || !name.trim()}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gradient-to-r from-[#f5a623] to-[#e07850] text-sm font-black text-white hover:opacity-90 transition disabled:opacity-40">
