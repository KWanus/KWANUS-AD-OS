...
import {
  ...
  AlertTriangle, TrendingUp, Layout,
} from "lucide-react";

interface HimalayaResults {
  mode: "scratch" | "improve";
  diagnosis: Record<string, unknown>;
  strategy: Record<string, unknown>;
  generated: Record<string, unknown>;
  created: { siteId?: string; emailFlowId?: string };
}
...
export default function HimalayaResultsPage() {
  const router = useRouter();
  const [results, setResults] = useState<HimalayaResults | null>(null);

  useEffect(() => {
    const stored = sessionStorage.getItem("himalaya_results");
    if (!stored) {
  ...
  const { mode, generated, strategy, created } = results;
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-3 pb-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-medium">
            ...
            {mode === "scratch" ? "Foundation built" : "Diagnosis complete"}
          </div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">
            {mode === "scratch" ? "Your Business Foundation" : "Your Business Audit & Fixes"}
          </h1>
          <p className="text-white/40 text-sm">
            {strat && (strat as Record<string, string>).summary}
          </p>
        </div>

        {/* ── Improve mode: Audit score ────────────────────────────────── */}
        {mode === "improve" && audit && (
        ...
        {homepage && (
          <Section title={mode === "improve" ? "Improved Homepage" : "Homepage Copy"} icon={Layout}>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <CopyButton text={`${homepage.headline}\n${homepage.subheadline}`} />
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4">
          ...
          <button
            onClick={() => { sessionStorage.removeItem("himalaya_results"); router.push("/himalaya"); }}
