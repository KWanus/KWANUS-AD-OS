    ...
    <div className={`rounded-3xl border p-5 sm:p-6 ${
      ...
      {isPrimary && (
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-[#f5a623]">Recommended Path</span>
        </div>
      )}

      <div className="flex items-start gap-4 mb-4">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
          isPrimary ? "bg-[#f5a623]/15" : "bg-white/[0.04]"
        }`}>
          <Icon className={`w-5 h-5 ${isPrimary ? "text-[#f5a623]" : "text-white/30"}`} />
        </div>
        ...
        <ol className="space-y-1.5">
          {rec.nextSteps.map((s, i) => (
            <li key={i} className="flex items-start gap-2.5 text-xs text-white/50">
              <span className="text-[#f5a623]/50 font-black shrink-0">{i + 1}.</span>
              {s}
            </li>
          ))}
        </ol>
      ...
      <div className="min-h-screen bg-t-bg text-white">
        ...
        <div className="mx-auto flex min-h-[50vh] max-w-3xl flex-col justify-center gap-4 px-4">
          <div className="flex flex-col items-center gap-4 rounded-2xl border border-white/[0.07] bg-white/[0.03] p-8">
            ...
            <Link href="/himalaya" className="text-sm text-[#f5a623] hover:text-[#f5a623]">← Start Over</Link>
          </div>
        </div>
      </div>
    );
