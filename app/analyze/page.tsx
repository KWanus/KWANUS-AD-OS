        ...
        {tabs.map((t) => (
          <button
            ...
            className={`px-3 py-2 text-xs font-semibold rounded-t-lg whitespace-nowrap transition ${activeTab === t.id
              ? "bg-white/10 text-white border-b-2 border-[#f5a623]"
...
function BriefCard({ brief, onOpenStudio }: { brief: AdBrief; onOpenStudio: () => void }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 overflow-hidden shadow-2xl">
      {/* Brief Header */}
      <div className="px-6 py-5 border-b border-white/10 bg-gradient-to-r from-[#f5a623]/10 to-transparent">
        ...
        {result.verdict !== "Reject" && assets && (
          savedCampaignId ? (
            <Link
              ...
              className="rounded-lg border border-[#f5a623]/30 bg-[#f5a623]/10 px-4 py-1.5 text-xs font-semibold text-[#f5a623] hover:bg-[#f5a623]/20 transition"
            >
              Open Workspace →
            </Link>
          ) : (
              ...
              {savedCampaignId ? (
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    autoFocus
                    className="w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs text-white placeholder-white/20 outline-none focus:border-[#f5a623]/60 transition"
                  />
                  <div className="flex gap-2">
                    <button
