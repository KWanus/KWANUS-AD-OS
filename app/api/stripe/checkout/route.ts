...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!stripeKey) {
      return NextResponse.json({ ok: false, error: "Stripe not configured" }, { status: 402 });
    }
    ...
    const body = await req.json() as { bundle: string };
    if (!body.bundle || !(body.bundle in CREDIT_BUNDLES)) {
      return NextResponse.json({ ok: false, error: "Invalid bundle" }, { status: 400 });
    }
    const bundle = CREDIT_BUNDLES[body.bundle as BundleKey];
