...
const TYPE_COLORS: Record<string, string> = {
  funnel: "text-cyan-400 bg-cyan-500/10 border-cyan-500/20",
  email_sequence: "text-purple-400 bg-purple-500/10 border-purple-500/20",
    ...
    <div className="min-h-screen bg-[#050a14] text-white">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="text-center mb-10">
          <div className="w-16 h-16 rounded-3xl bg-gradient-to-br from-cyan-500 to-purple-600 flex items-center justify-center mx-auto mb-4 shadow-[0_0_40px_rgba(6,182,212,0.3)]">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="flex-1 relative">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search marketplace..."
              className="w-full bg-white/[0.04] border border-white/[0.1] rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-cyan-500/50 transition" />
          </div>
          <div className="flex gap-2">
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
              ...
              <option value="popular" className="bg-[#0a0f1e]">Most Popular</option>
              <option value="rating" className="bg-[#0a0f1e]">Highest Rated</option>
              <option value="price_low" className="bg-[#0a0f1e]">Price: Low</option>
              <option value="price_high" className="bg-[#0a0f1e]">Price: High</option>
            </select>
          </div>
        </div>
          ...
          {types.map((type) => (
            <button key={type} onClick={() => setFilterType(type)}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition border ${
                filterType === type ? "border-cyan-500/40 bg-cyan-500/10 text-cyan-300" : "border-white/10 bg-white/[0.03] text-white/30"
              }`}>
            ...
            {filtered.map((item) => {
              ...
              return (
                <div key={item.id} className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden hover:border-cyan-400/20 transition group">
                  {item.featured && (
                    <div className="bg-gradient-to-r from-cyan-500 to-purple-600 px-4 py-1 text-[10px] font-bold text-white text-center">
                      Featured
                    </div>
                  )}
                  <div className="p-5">
                    ...
                    <button className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-600 text-white text-xs font-bold opacity-0 group-hover:opacity-100 transition">
