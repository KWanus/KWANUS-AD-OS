  ...
  return (
    <div
      className={`relative min-w-[220px] rounded-2xl border transition-all duration-200 ${
        selected
          ? "border-[#e07850]/80 shadow-[0_0_32px_rgba(168,85,247,0.4)]"
          : "border-[#e07850]/30 shadow-[0_0_16px_rgba(168,85,247,0.15)]"
      }`}
      style={{ background: "linear-gradient(135deg, rgba(168,85,247,0.10) 0%, rgba(109,40,217,0.07) 100%)" }}
    >
      {/* Top glow */}
      <div className="absolute inset-x-0 top-0 h-0.5 rounded-t-2xl bg-gradient-to-r from-transparent via-[#e07850] to-transparent" />

      <div className="px-4 py-3">
        {/* Header */}
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            <Mail className="w-3 h-3 text-[#e07850]" />
            <span className="text-[10px] font-black uppercase tracking-widest text-[#e07850]/70">Send Email</span>
          </div>
          <div className="flex items-center gap-1">
            <button
              onClick={() => nodeData.onEdit?.(id)}
              className="p-1 rounded-md hover:bg-[#e07850]/20 text-white/30 hover:text-[#f5a623] transition"
              title="Edit email"
            >
        ...
        <div className="flex items-start gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-[#e07850]/15 border border-[#e07850]/20 flex items-center justify-center shrink-0">
            <Mail className="w-4 h-4 text-[#e07850]" />
          </div>
      ...
      <Handle
        ...
        className="!w-3 !h-3 !border-2 !border-[#e07850] !bg-[#0c0a08] hover:!bg-[#e07850] transition-colors"
        style={{ top: -6 }}
      />
      <Handle
        type="source"
